...
namespace Rocket_Elevators_Customer_Portal.Controllers
{
    public class ProductManagementController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("https://donnguyen.herokuapp.com/api/Customers/" + userName))
                {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("https://donnguyen.herokuapp.com/api/Buildings/CustomerId/" + customers[0].Id))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
            ...
            foreach (var building in allBuildings)
            {
                using (var httpClient = new HttpClient())
                {
                    using (var response = await httpClient.GetAsync("https://donnguyen.herokuapp.com/api/Batteries/buildingId/" + building.Id))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
            ...
            foreach (var battery in allBatteries)
            {
                using (var httpClient = new HttpClient())
                {
                    using (var response = await httpClient.GetAsync("https://donnguyen.herokuapp.com/api/Columns/batteriesId/" + battery.Id))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
            ...
            foreach (var column in allColumns)
            {
                using (var httpClient = new HttpClient())
                {
                    using (var response = await httpClient.GetAsync("https://donnguyen.herokuapp.com/api/Elevators/columnId/" + column.Id))
                    {
                        string apiResponse = await response.Content.ReadAsStringAsync();
